...
interface DocumentViewProps {
    ...
    onClose: () => void;
    storeName: string;
    qrisImageUrl?: string;  // tetap diterima dari parent untuk backward compatibility, tapi tidak ditampilkan
...
export default function DocumentViewerModal({
    ...
    // Handle URL: support both full Cloudinary URLs (https://...) and legacy paths (/uploads/...)
    const API_BASE_URL = import.meta.env.VITE_API_BASE_URL?.replace('/api', '') || 'http://localhost:5000';
    const fullIdCardUrl = idCardImageUrl
        ? (idCardImageUrl.startsWith('http') ? idCardImageUrl : `${API_BASE_URL}${idCardImageUrl}`)
        : undefined;
